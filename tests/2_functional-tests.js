...
suite('Functional Tests', function() {
  suite('GET /api/issues/:project', function() {
    ...
    test('Should view issues on a project with one filter: issue_text', function(done) {
      chai
        .request(server)
        .get('/api/issues/alex-api?issue_text=' + issueTextFilter)
        .set('Content-Type', 'application/json')
        .end(function(err, res) {
          assert.equal(res.status, 200);
    ...
    test('Should view issues on a project with one filter: issue_text, created_by', function(done) {
      chai
        .request(server)
        .get('/api/issues/alex-api?issue_text=' + issueTextFilter + '&created_by=' + createdByFilter)
        .set('Content-Type', 'application/json')
        .end(function(err, res) {
          assert.equal(res.status, 200);
          assert.equal(res.body[0].issue_text, issueTextFilter);
          ...
          chai
            ...
            .then(() => {
              chai
                .request(server)
                .keepOpen()
                .get('/api/issues/alex-api?_id=' + postRes.body._id)
                .set('Content-Type', 'application/json')
  ...
  suite('DELETE /api/issues/:project', function() {
    test('Should delete an issue', function(done) {
      fetch(remoteDbUri, {
        ...
        body: JSON.stringify(baseTestIssue)
      }).then((postRes) => {
        return postRes.json();
      }).then((issueToDelete) => {
        chai
        .request(server)
        .delete('/api/issues/alex-api')
        .send({
